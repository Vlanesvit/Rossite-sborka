...
document.addEventListener("DOMContentLoaded", function () {
	...
	menuItems.forEach(parent => {
		...
		// Создаём кнопку для открытия подменю
		const toggleButton = document.createElement("button");
		toggleButton.classList.add("menu-toggle");
		toggleButton.setAttribute("aria-label", "toggle submenu");
		parent.insertBefore(toggleButton, link.nextSibling);
		...
		toggleButton.addEventListener("click", function (event) {
			event.stopPropagation(); // Чтобы клик не доходил до родителя
...
(() => {
	...
	class Popup {
		...
		open(selectorValue) {
			if (bodyLockStatus) {
				...
				if (this.targetOpen.element) {
					...
					// Открываем новое модальное окно, не закрывая предыдущее
					this.targetOpen.element.classList.add(this.options.classes.popupActive);
					document.documentElement.classList.add(this.options.classes.bodyActive);
					this.targetOpen.element.setAttribute('aria-hidden', 'false');

					// Блокировка скролла
					if (!this.isOpen) { // Блокируем скролл, если еще не было модальных окон
						document.documentElement.classList.add('lock');
					}

					// Добавляем модальное окно в стек открытых окон
					if (!this.openPopups) this.openPopups = [];
					this.openPopups.push(this.targetOpen.element);
		...
		close(selectorValue) {
			...
			let popupToClose = selectorValue
				? document.querySelector(selectorValue)
				: this.openPopups.pop(); // Закрываем только последнее открытое окно

			if (popupToClose) {
				// Добавляем класс для анимации закрытия
				popupToClose.classList.add('popupClosing');

				// После завершения анимации скрываем окно
				setTimeout(() => {
					popupToClose.classList.remove(this.options.classes.popupActive);
					popupToClose.classList.remove('popupClosing');
					popupToClose.setAttribute('aria-hidden', 'true');
				}, 300);  // Задержка должна совпадать с длительностью анимации

				// Если больше нет открытых окон, разблокируем скролл
				if (this.openPopups.length === 0) {
					document.documentElement.classList.remove('lock'); // Разблокировка прокрутки
				}
			}
...
window.addEventListener('load', addAnimText());
/* ====================================
Кастомный курсор
==================================== */
...
document.addEventListener('DOMContentLoaded', function () {
	// Список CSS-переменных с описаниями
	const cssVariables = [
		{ name: '--bg-color', desc: 'Основной цвет фона' },
		{ name: '--font-color', desc: 'Цвет основного текста' },
		{ name: '--text-color', desc: 'Цвет дополнительного текста' },
		{ name: '--title-color', desc: 'Цвет заголовков' },
		{ name: '--primary-color', desc: 'Основной бренд-цвет' },
		{ name: '--primary-opacity-color', desc: 'Прозрачная версия основного цвета' },
		{ name: '--primary-hover', desc: 'Цвет ховера для основных элементов' },
		{ name: '--gray-color', desc: 'Серый цвет элементов' },
		{ name: '--gray-opacity-color', desc: 'Прозрачный серый цвет' },
		{ name: '--light-color', desc: 'Светлый цвет фона' },
		{ name: '--light-color-opacity', desc: 'Прозрачный светлый цвет' },
		{ name: '--icons-color-dark', desc: 'Цвет иконок (темный)' },
		{ name: '--icons-color-light', desc: 'Цвет иконок (светлый)' },
		{ name: '--icons-opacity-color', desc: 'Прозрачный цвет иконок' },
		{ name: '--icons-menu-color', desc: 'Цвет иконок меню' },
		{ name: '--icons-button-white', desc: 'Цвет кнопочных иконок' },
		{ name: '--icons-button-opacity-white', desc: 'Прозрачные кнопочные иконки' },
		{ name: '--icon-vk', desc: 'Цвет иконки VK' },
		{ name: '--icon-tg', desc: 'Цвет иконки Telegram' },
		{ name: '--icon-wt', desc: 'Цвет иконки WhatsApp' },
		{ name: '--icon-inst', desc: 'Цвет иконки Instagram' },
		{ name: '--icon-ok', desc: 'Цвет иконки Odnoklassniki' },
		{ name: '--icon-yt', desc: 'Цвет иконки YouTube' },
		{ name: '--box-shadow', desc: 'Тень элементов' },
		{ name: '--page-border-radius', desc: 'Радиус скругления' }
	];
	...
	// Получение альфа-канала из цвета
	function getAlphaFromColor(color) {
		if (color.startsWith('rgba')) {
			const alpha = color.split(',')[3].trim().replace(')', '');
	...
	function setAlphaToColor(color, alpha) {
	...
	function createControlPanel() {
		...
		content.style.maxHeight = 'calc(80vh - 160px)';

		// Создание элементов управления
		cssVariables.forEach(({ name, desc }) => {
			...
			const savedValue = localStorage.getItem('cssVariables')
				? JSON.parse(localStorage.getItem('cssVariables'))[name]
				: null;

			const currentValue = savedValue ||
				getComputedStyle(document.documentElement)
					.getPropertyValue(name)
					.trim();

			// Создание элементов ввода
			if (/^(#|rgb|hsl)/.test(currentValue)) {
				...
				colorInput.value = currentValue.startsWith('#')
					? currentValue
					: (rgbToHex(currentValue) || '#000000');

				// Управление прозрачностью
				const alphaContainer = document.createElement('div');
				alphaContainer.style.display = 'flex';
				alphaContainer.style.gap = '8px';
				...
				alphaInput.value = getAlphaFromColor(currentValue);
				...
				alphaText.max = 1;
				alphaText.step = 0.01;
				alphaText.value = alphaInput.value;
				alphaText.style.width = '60px';
				alphaText.style.padding = '3px';

				// Обновление цвета
				const updateColor = () => {
					let colorValue;
					if (colorInput.value.startsWith('#')) {
						colorValue = hexToRgba(colorInput.value, alphaInput.value);
					} else {
						colorValue = setAlphaToColor(colorInput.value, alphaInput.value);
					}
				...
				alphaText.addEventListener('input', () => {
					const value = Math.min(Math.max(alphaText.value, 0), 1);
		...
		closeButton.addEventListener('click', () => {
			...
			document.getElementById('style-toggle-button').style.display = 'block';
		});
		...
		collapseBtn.addEventListener('click', () => {
			...
			panel.style.height = isCollapsed ? '60px' : 'auto';
			panel.style.overflow = isCollapsed ? 'hidden' : 'visible';
		});
	...
	// Обновление предпросмотра CSS
	function updateCodePreview() {
		const variables = {};
		cssVariables.forEach(({ name }) => {
			variables[name] = getComputedStyle(document.documentElement)
				.getPropertyValue(name)
				.trim();
		});
		...
		document.querySelector('#style-control-panel pre').textContent = cssCode;
	}

	// Логика перетаскивания
	function startDragging(e) {
		isDragging = true;
		const panel = document.getElementById('style-control-panel');
		dragStartX = e.clientX;
		dragStartY = e.clientY;
		elemStartX = panel.offsetLeft;
		elemStartY = panel.offsetTop;
	}
	...
	function drag(e) {
		...
		panel.style.left = `${elemStartX + deltaX}px`;
		panel.style.top = `${elemStartY + deltaY}px`;
	}
	...
	function saveVariables() {
		...
		localStorage.setItem('cssVariables', JSON.stringify(variables));
	}

	// Загрузка переменных
	function loadVariables() {
		const saved = localStorage.getItem('cssVariables');
		if (saved) {
	...
	function createToggleButton() {
		const toggleButton = document.createElement('button');
		toggleButton.id = 'style-toggle-button';
		toggleButton.textContent = '⚙️ Open Style Editor';
