...
(() => {
	"use strict";
	const modules = {};
	// Добавим глобальный флаг для блокировки скролла, если его не было
	...
	class Popup {
		constructor(options) {
			...
			this.openPopups = []; // стек открытых попапов
		...
		eventsPopup() {
			...
			// 		window.location.hash ? this._openToHash() : this.close();
			// 	});
		...
		close(selectorValue) {
			...
			const popupToClose = selectorValue
